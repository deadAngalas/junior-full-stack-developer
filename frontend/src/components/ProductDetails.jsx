...
export default function ProductDetails() {
  ...
  const allSelected = allAttributes.every((attr) => selectedAttributes[attr.name]);
    ...
    <div className="product-details-three-col">
        ...
        <div className="gallery-main">
          {gallery.length > 0 && (
            <>
              ...
              <button className="gallery-arrow left" onClick={() => handleSlide(-1)}>
                &lt;
              </button>
              <button className="gallery-arrow right" onClick={() => handleSlide(1)}>
                &gt;
              </button>
            </>
          )}
      ...
      <div className="product-info-panel">
        ...
        {allAttributes.map((attr) => {
          ...
          return (
            ...
            >
              ...
              <div className={`attr-items-row ${isColor ? "color-options" : "text-options"}`}>
                {items.map((item) => {
        ...
        <div className="product-desc" data-testid="product-description">
          {product.description ? parse(product.description) : <em>No description.</em>}
        </div>
      </div>
    </div>
  );
